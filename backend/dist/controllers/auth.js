...
const generateAccessToken = (userId) => {
    return jwt.sign({ id: userId }, process.env.JWT_SECRET || 'fm_access_secret', { expiresIn: getAccessExpiration() });
};
const generateRefreshToken = (userId) => {
    return jwt.sign({ id: userId }, process.env.JWT_REFRESH_SECRET || 'fm_refresh_secret', { expiresIn: getRefreshExpiration() });
};
...
export const refresh = async (req, res, next) => {
    try {
        ...
        // Verify token
        const decoded = jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET || 'fm_refresh_secret');
